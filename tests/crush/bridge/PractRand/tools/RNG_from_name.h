...
namespace Special_RNGs {
	...
	class _stdin_reader {
		...
		void refill() {
			// Use POSIX read() instead of fread() to avoid macOS ARM
			// buffered-IO issues with binary stdin.
			std::size_t total = 0;
			std::size_t want = BUFF_SIZE * sizeof(Word);
			char *dst = reinterpret_cast<char*>(&buffer[0]);
			while (total < want) {
				ssize_t got = ::read(STDIN_FILENO, dst + total, want - total);
				if (got <= 0) { ended = true; break; }
				total += got;
			}
			std::size_t n = total / sizeof(Word);
			if (!n) read_failed();
			pos = &buffer[0];
			end = &buffer[n];
		}
