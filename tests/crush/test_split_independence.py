...
def test_split_independence_small():
    # quick exercise interleaving with 1GB total using two children of the same
    # parent (original scenario).
    out = _run_interleaved(n_uint32=1 << 30, tlmax="1GB")
...
def test_independent_parents_same_index():
    ...
    out = _run_interleaved(n_uint32=1 << 30, tlmax="1GB", parent_seeds=(1, 2), indices=(0, 0))
    fails = _parse_practrand_failures(out)
...
def test_independent_parents_diff_index():
    ...
    out = _run_interleaved(n_uint32=1 << 30, tlmax="1GB", parent_seeds=(1, 2), indices=(0, 1))
    fails = _parse_practrand_failures(out)
