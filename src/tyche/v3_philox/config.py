...
class TycheV3_PhiloxConfig:
    def __init__(self, tile_size: int = 16, num_rounds: int = 10, word_size: int = 32, embedding: str = "hash", backend: str = "pallas"):
        if backend == "pallas":
            from tyche.v3_philox.backend_pallas import PallasBackendV3_Philox
            self._backend = PallasBackendV3_Philox(num_rounds, tile_size, word_size)
    ...
    @property
    def name(self):
        return f"tyc_phi_w{self.word_size}_t{self.tile_size}_r{self.num_rounds}_{self.embedding}"
