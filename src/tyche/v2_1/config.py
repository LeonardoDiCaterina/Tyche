...
from tyche.v2_1.algorithm import (
    key_size_uint32,
    expand_seed_to_key,
    make_hash_parallel,
    make_tiles,
...
class TycheV2_1Config:
    def __init__(self, tile_size: int = 16, num_rounds: int = 4, embedding: str = "hash", backend: str = "jax"):
        if backend == "pallas":
            from tyche.v2_1.backend_pallas import PallasBackendV2_1
            self._backend = PallasBackendV2_1(num_rounds, tile_size)
    ...
    @property
    def name(self):
        return f"tyc21_t{self.tile_size}_r{self.num_rounds}_{self.embedding}"
