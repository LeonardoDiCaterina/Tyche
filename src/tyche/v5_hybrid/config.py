...
class TycheV5Config:
    def __init__(self, tile_size: int = 16, num_rounds: int = 1, embedding: str = "hash", backend: str = "cuda"):
        if backend != "cuda":
            raise ValueError("Tyche V5 Hybrid only supports backend='cuda'!")
        if tile_size != 16:
            raise ValueError("Tyche V5 Hybrid MVP only supports tile_size=16")
        if num_rounds != 1:
            raise ValueError("Tyche V5 Hybrid MVP only supports num_rounds=1")

        from tyche.v5_hybrid.backend_cuda import CudaBackendV5
        self._backend = CudaBackendV5(num_rounds, tile_size)
